...
determine_relay_user ()
{
	...
	} else if (relay_method == METHOD_HTTPS) {
		if (user == NULL)
		    user = getparam (ENV_HTTPS_PROXY_USER);
	}
	if (user == NULL)
...
determine_relay_password ()
{
	...
	if (pass == NULL && relay_method == METHOD_HTTPS)
		pass = getparam(ENV_HTTPS_PROXY_PASSWORD);
...
set_relay( int method, char *spec )
{
	...
	case METHOD_HTTPS:
		if ( spec == NULL )
		    spec = getparam(ENV_HTTPS_PROXY);
		if ( spec == NULL )
		    fatal("You must specify https proxy server\n");
		relay_port = 443;                        /* set default first */
		break;
	case METHOD_TELNET:
		if ( spec == NULL )
	...
	if (expect( spec, HTTP_PROXY_PREFIX)) {
		...
		buf[strcspn(buf, "/")] = '\0';
	} else if (expect( spec, HTTPS_PROXY_PREFIX)) {
		/* URL format like: "https://server:port/" */
		/* extract server:port part */
		buf = strdup( spec + strlen(HTTPS_PROXY_PREFIX));
		buf[strcspn(buf, "/")] = '\0';
	} else {
	...
	while ( (0 < argc) && (**argv == '-') ) {
		...
		while ( ptr && *ptr )
		{
		    ...
		    case 'H':                           /* specify http-proxy server */
		        ...
		        break;
		    case 'X':                           /* specify https-proxy server */
		        if ( 1 < argc ) {
					argc--;
					argv++;
		            method = METHOD_HTTPS;
			...
			case '-':							/* long options */
				...
				if (!strcmp(*argv, "--socks-proxy"))
				{
					...
					{
						error("option '%s' needs argument.\n", *argv);
						err++;
					}
				}
				else if (!strcmp(*argv, "--http-proxy"))
				{
					if ( 1 < argc )
					{
						argc--;
				...
				else if (!strcmp(*argv, "--https-proxy"))
				{
					if ( 1 < argc )
					{
						argc--;
				...
				else if (!strcmp(*argv, "--https-proxy-ca"))
				{
					if ( 1 < argc )
					{
						argc--;
				...
				else if (!strcmp(*argv, "--https-proxy-ca-path"))
				{
					if ( 1 < argc )
					{
						argc--;
				...
				else if (!strcmp(*argv, "--no-check-certificate"))
				{
					https_check_certificate = 0;
				}
				else if (!strcmp(*argv, "--https-proxy-certname"))
				{
					if ( 1 < argc )
					{
						argc--;
						argv++;
						https_proxy_certname = *argv;
					}	
					else
					{
						error("option '%s' needs argument.\n", *argv);
				...
				else if (!strcmp(*argv, "--https-user-cert"))
				{
					if ( 1 < argc )
					{
						argc--;
						argv++;
						https_usercert_file = *argv;
					}	
					else
					{
						error("option '%s' needs argument.\n", *argv);
				...
				else if (!strcmp(*argv, "--https-user-key"))
				{
					if ( 1 < argc )
					{
						argc--;
						argv++;
						https_userkey_file = *argv;
					}	
					else
					{
						error("option '%s' needs argument.\n", *argv);
				...
				else if (!strcmp(*argv, "--telnet-proxy"))
				{
					if ( 1 < argc )
					{
						argc--;
...
/* SSL callback routing for reading private key passphrase, if necessary */
int password_callback (char *buf, int size, int rwflag, void *u)
{
	...
	if (buf)
	{    
		pass = readpass("Enter privatekey passphrase for\n%s: ", https_userkey_file);
		if (pass)
		{
...
int
socks5_auth_parse(char *start, int *auth_list, int max_auth)
{
	char *end;
	int i = 0;
	while ( i < max_auth ) {
...
begin_socks5_relay( SOCKET s )
{
	...
	unsigned char n_auth = 0, auth_method;
	int len, auth_list[10], auth_result, i;
...
begin_socks4_relay( SOCKET s )
{
	...
	*/
	...
	if ( (socks_version == 4) && (dest_addr.sin_addr.s_addr == 0)) {
		strncpy( ptr, dest_host, sizeof(buf)+buf-ptr );
...
sendf(SOCKET s, const char *fmt,...)
{
	...
	else
	{
		...
		{
		    debug("failed to send https request.\n");
		    ssl_error( "SSL_write()" );
		    return -1;
		}
	}
...
sslread(char *buf, size_t size)
{
	...
	debug("SSL_read: read %d bytes\n", ret);
	if (ret < 0)
	{
		debug("failed receiving https reply.\n");
		ssl_error( "SSL_read()" );
		return -1;
	}
	else
...
int
ssl_line_input(char *buf, size_t size)
{
	char *dst = sslbuf;
	size_t len = 0;
	...
	/* global buffer empty, fill it with new data  and reset the index */
	if (strlen(sslbuf) == 0)
	{
		if (sslread(sslbuf, sizeof(sslbuf)) < 0)
		{
			debug("no reply\n");
		    return -1;
		}
	...
	dst += sslbuf_idx;
	while (*dst != '\n' && sslbuf_idx+len < sizeof(sslbuf) && len < size)
	{
		dst++;
		len++;
	}
	...
	/* no valid input found, or input too long for buffer */
	if ((len > size) || (*dst != '\n'))
		return -1;
...
begin_http_relay( SOCKET s )
{
	...
	if (sendf(s,"CONNECT %s:%d HTTP/1.1\r\nHost: %s\r\n", dest_host, dest_port, dest_host) < 0)
		return START_ERROR;
...
/* begin relaying via HTTPS proxy
   Directs CONNECT method to proxy server to connect to
   destination host (and port). It may not be allowed on your
   proxy server.
 */
int
begin_https_relay( SOCKET s )
{
	char buf[4096];
	int result;
	char *auth_what;
	...
	debug("begin_https_relay()\n");

	if (https_proxy_certname == NULL)
		https_proxy_certname = relay_host;
	debug("https_proxy = %s\n", relay_host);

 	if (https_proxy_certname == NULL) 
 		https_proxy_certname = getparam(ENV_HTTPS_PROXY_CERTNAME);
 	if (https_ca_file == NULL) 
 		https_ca_file = getparam(ENV_HTTPS_PROXY_CA_FILE);
 	if (https_ca_path == NULL) 
 		https_ca_path = getparam(ENV_HTTPS_PROXY_CA_PATH);
 	if (https_usercert_file == NULL) 
 		https_usercert_file = getparam(ENV_HTTPS_PROXY_USERCERT);
 	if (https_userkey_file == NULL) 
 		https_userkey_file = getparam(ENV_HTTPS_PROXY_USERKEY);

	debug("https_proxy_certname = %s\n", https_proxy_certname);
	debug("https_ca_file = %s\n", https_ca_file);
	debug("https_ca_path = %s\n", https_ca_path);
	debug("https_usercert = %s\n", https_usercert_file);
	debug("https_userkey = %s\n", https_userkey_file);
	...
	if (SSL_CTX_load_verify_locations(ctx, https_ca_file, https_ca_path) != 1)
		ssl_error( "SSL_CTX_load_verify_locations()" );
	if (https_check_certificate)
	{
#ifdef X509_CHECK_FLAG_ALWAYS_CHECK_SUBJECT
		debug( "Adding https proxy server certificate check\n" );
		param = X509_VERIFY_PARAM_new();
	 	if (X509_VERIFY_PARAM_set1_host(param, https_proxy_certname, strlen(https_proxy_certname)) != 1)
			ssl_error( "X509_VERIFY_PARAM_set1_host()" );
		if (SSL_CTX_set1_param(ctx, param) != 1)
			ssl_error( "SSL_CTX_set1_param()" );
		X509_VERIFY_PARAM_free(param);
#else
		debug( "Your version of OpenSSL only has limited server certificate check support!" );
#endif
	...
	SSL_CTX_set_default_passwd_cb (ctx, password_callback);
 	...
 	if (https_usercert_file && SSL_CTX_use_certificate_chain_file(ctx, https_usercert_file) <= 0)
		ssl_error( "SSL_CTX_use_certificate_chain_file" );
 	if (https_userkey_file && SSL_CTX_use_PrivateKey_file(ctx, https_userkey_file, SSL_FILETYPE_PEM) <= 0)
		ssl_error( "SSL_CTX_use_PrivateKey_file" );

	if ((ssl = SSL_new(ctx)) == NULL) 
		ssl_error( "SSL_new()" );
	if (SSL_set_fd(ssl, s) == 0)
		ssl_error( "SSL_set_fd()" );
	if ((result = SSL_connect(ssl)) <= 0)
	...
	if (!strchr(buf, ' '))
	{
		error ("Unexpected https response: '%s'.\n", buf);
		return START_ERROR;
	}
	result = atoi(strchr(buf,' '));
	...
	case 302:                                   /* redirect */
		do {
		    if (ssl_line_input(buf, sizeof(buf)))
		        break;
		    if (expect(buf, "Location: ")) {
		        relay_host = cut_token(buf, "//");
		        cut_token(buf, "/");
	...
	case 407:                                   /* Proxy-Auth required */
		...
		do {
		    ...
		    if (expect(buf, auth_what)) {
		        ...
		        realm = cut_token(scheme, " ");
		        if ( scheme == NULL || realm == NULL ) {
		            debug("Invalid format of %s field.\n", auth_what);
		            return START_ERROR;         /* fail */
		        }
		        /* check supported auth type */
		        if (expect(scheme, "basic")) {
		            ...
		            debug("Unsupported authentication type: %s\n", scheme);
		        }
		    }
		} while (strcmp(buf,"\r\n") != 0);
		if ( proxy_auth_type == PROXY_AUTH_NONE ) {
		    debug("Can't find %s in response header.\n", auth_what);
		    return START_ERROR;
		} else {
			debug("got a 401/407 but going for the retry!\n");
			return START_RETRY;
		}
	...
	default:
		/* Not allowed */
		debug("https proxy is not allowed.\n");
		return START_FORBIDDEN;
	}
	/* skip to end of response header */
	do
...
/* relay byte from stdin to socket and fro socket to stdout.
   returns reason of termination */
int
do_ssl_repeater( SOCKET local_in, SOCKET local_out, SOCKET remote)
{
	/** vars for local input data **/
	char lbuf[4096];                            /* local input buffer */
	int lbuf_len;                               /* available data in lbuf */
	...
	while ( f_local || f_remote ) {
		...
		/* remote => local */
		if ( FD_ISSET(remote, &ifds) && (rbuf_len < (int)sizeof(rbuf)) ) {
		    len = sslread(rbuf + rbuf_len, sizeof(rbuf)-rbuf_len);
		    if ( len == 0 || (len == -1 && socket_errno() == ECONNRESET)) {
		        debug("connection %s by peer\n",
		              (len==0)? "closed": "reset");
		        close_reason = REASON_CLOSED_BY_REMOTE;
		...
		/* flush data in buffer to socket */
		if ( 0 < lbuf_len ) {
		    len = SSL_write(ssl, lbuf, lbuf_len);
		    if ( len == -1 ) {
		        fatal("send() failed, %d\n", socket_errno());
		    } else if ( 0 < len ) {
		        if ( 1 < f_debug )              /* more verbose */
...
retry:
	...
	case METHOD_HTTP:
		...
		switch (ret) {
		case START_FORBIDDEN:
		    close (remote);
		    fatal("failed to begin relaying via HTTP (Forbidden).\n");
		case START_ERROR:
		    close (remote);
		...
		break;
	case METHOD_HTTPS:
		ret = begin_https_relay(remote);
		switch (ret) {
		case START_FORBIDDEN:
		    close (remote);
		    fatal("failed to begin relaying via HTTPS (Forbidden).\n");
		case START_ERROR:
		    close (remote);
		    fatal("failed to begin relaying via HTTPS.\n");
		case START_OK:
		    break;
		case START_RETRY:
		    /* retry with authentication */
...
do_repeater:
	if (relay_method == METHOD_HTTPS)
	{
		reason = do_ssl_repeater(local_in, local_out, remote);
	}
	else
	{
	...
	debug ("relaying done.\n");
	if (local_type == LOCAL_SOCKET && reason == REASON_CLOSED_BY_LOCAL && f_hold_session)
	{
...
/* ------------------------------------------------------------
   Local Variables:
   compile-command: "gcc -Wall -Wpedantic  -o connect connect.c -lssl -lcrypto"
   tab-width: 8
   fill-column: 74
   comment-column: 48
   End:
